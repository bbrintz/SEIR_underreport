...
tt <- cmdstan_model("SEIR_betabin_on_hier_ar1_beta_pbeta_zeros_v16_pois.stan")
...
out_dir <- "/private/tmp/seir_underreport_v16_pois_boundary_ei"
...
fit$summary(c("eta_p_time", "average_p", "log_initial_ei_count", "initial_ei"))
...
mcmc_pairs(fit$draws(c("p", "sigma", "sig_beta[1]", "initial_ei[1]", "initial_ei[2]")),
           np = np_fit,
           pars = c(p_reference, "sigma", "sig_beta[1]", "initial_ei[1]", "initial_ei[2]"),
                  off_diag_args = list(size = 0.75))

#fit$output_files()=paste0(getwd(),"/",new_csv)

# To reload a previous v16 Poisson run, set new_csv to one of its output CSV files.
# new_csv <- paste0(out_dir, "/SEIR_betabin_on_hier_ar1_beta_pbeta_zeros_v16_pois-<timestamp>-1-<id>.csv")
