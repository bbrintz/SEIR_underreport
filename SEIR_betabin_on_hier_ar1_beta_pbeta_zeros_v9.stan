...
transformed parameters {
  ...
  log_beta = rep_vector(0,TT);
  beta_mat = rep_matrix(0,TT,N_C);
  log_beta_mat = rep_matrix(0,TT,N_C);

  log_beta[t0] = mu_log_beta
                + sigma / sqrt(1 - phi^2) * z[t0];
  for (n in (t0+1):TT)
  ...
  for (ct in 1:N_C) {
    for (n in 2:TT) {
      if (n > first[ct] && n <= (last[ct]+1)) {
            ...
            // Preserve the seeded E->I on the first step; then update only when exposure exists
            if ((n - 1) == first[ct]) {
...
model {
  ...
  v_raw ~ normal(logit(0.7), 0.3);//normal(0,1); //logistic(0,1);

  mu_log_beta ~ normal(log(.5),1);
