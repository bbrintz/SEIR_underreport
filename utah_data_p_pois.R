...
tt <- cmdstan_model("SEIR_betabin_on_hier_ar1_beta_pbeta_zeros_v12_pois.stan")#cmdstan_model("SEIR_betabin_vary_beta_nospat.stan")#"stoch_beta_spatial_SI_utah_betabin.stan")
...
# Initialize e0 directly for the exposed fraction before each county's first week.

out_dir <- "/private/tmp/seir_underreport_v12_pois"
...
fit = tt$sample(data = dat, chains = 4,
                 ...
                 init = \() {list(u_t_logit_eta = matrix(rnorm(TT*N_C, 0,1), TT, N_C),
                                  ...
                                  #raw_log_beta_mat = matrix(runif(TT*N_C, -1, 1), TT, N_C),#matrix(rnorm(TT*N_C, 0, 1), TT, N_C),
                                  raw_log_beta = rnorm(sum(dat$last - dat$first + 1)),
                                  eta_p_time = rlogis(ceiling(TT / dat$N_wks_per_period)),
                                  #kappa = runif(1, 0,1),
                                  v_raw=runif(1,1.5,2.5),
                                  z=rnorm(TT-min(first) + 1,0,.25),
                                  mu_log_beta = rnorm(1, 0, .05),
                                  ...
                                  log_e0_count = rnorm(N_C, log(20), 0.5),
                                  #rho_si = runif(1, 0.0001, 0.005),
...
fit$summary(c("eta_p_time", "average_p"))
...
mcmc_pairs(fit$draws(c("p", "sigma", "sig_beta", "e0[1]", "e0[2]")), np = np_fit, pars = c("p[1,1]", "sigma", "sig_beta[1]", "e0[1]", "e0[2]"),
                  off_diag_args = list(size = 0.75))

#fit$output_files()=paste0(getwd(),"/",new_csv)

# To reload a previous v12 Poisson run, set new_csv to one of its output CSV files.
# new_csv <- paste0(getwd(), "/tst_folder_sig/", "SEIR_betabin_on_hier_ar1_beta_pbeta_zeros_v12_pois-<timestamp>-1-<id>.csv")
