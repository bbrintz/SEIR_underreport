...
Despite these innovations, there are limitations to each approach.
The LNA approach approximates increments of the stochastic-disease-compartment-model-counting with an exponentiated multivariate Gaussian; a limitation to this approach is that the trajectories don't necessarily adhere to the population limits, namely that cumulative infections must be less than or equal to the population size. 
This issue may occur at the tail end of an epidemic.
Ideally, approximate inference methods for stochastic compartment models would respect the natural constraints imposed by finitely-sized populations.
...
(2) allowing an estimable time-evolving and country-specific transmission rate using a hierarchical AR(1) process , and  
(3) a continuous approximation that enables gradient-based Bayesian inference of dynamics in a discrete-time stochastic SEIR with beta-binomial transitions.
...
We approximate $se_{it}, ei_{it}, ir_{it}$ with scaled inverse-logits of normal random variables matched on mean and variance (see Appendix). The AR(1) on `\log\beta` is initialized at its stationary distribution in Stan using
...
## Simulation Study
We used simulated data to assess model performance in recovering true parameters and states. Similar to the Utah health district data, we simulated 30 weeks of data for 11 districts with known parameters. In 200 simulations of observed case counts per setting, we varied the detection probability ($p=0.15, 0.50, 0.85$), used true health district population data, and set true parameters to match posterior draws from the Utah data model fit.  Across simulated detection probabilities, we found the model recovered parameters well with low average error, high coverage, and reasonable credible interval widths across all detection probabilities. As expected, higher detection probabilities led to improved parameter recovery (@tbl-sim-results).
...
p_est %>% mutate(Value=round(Value,3)) %>% mutate(range=case_when(
  ...
  range=="201-400" ~ "Medium (0.50)",
  range=="401-600" ~ "High (0.85)"
)) %>% mutate(Key = case_when(
    Key=="p1" ~ "Average Error",
    Key=="p2" ~ "Coverage",
    Key=="p3" ~ "90% CrI Width"
)) %>% pivot_wider(names_from=Key, values_from=Value) %>% ungroup() %>% 
mutate(true=c(.15,.5,.85)) %>% mutate(perc=`Average Error`/true) %>% 
...
`Detection Probability`=range) %>%
kable(align=c("l","c","c","c"))
```

While our case study model fit had no divergences, the simulation study had divergences across detection probabilities (@tbl-div-results). On average, there were low divergence rates across all detection probabilities, with higher rates at higher detection probabilities. 
...
# Appendix: Continuous Approximation of Discrete Transitions

\section{Appendix: Inverse Logit calculation}
$$
\begin{aligned}
