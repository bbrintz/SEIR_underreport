...
We assume these realized cases represent a fraction of total incident cases, which are generated as increments in the underlying discrete-time stochastic transmission process.
...
The SEIR model is characterized as a multivariate counting process $\mathbf{N}_t$, comprising cumulative counts of movement of individuals in a population of size $N_{S,0}$ from susceptible-to-exposed (SE), exposed-to-infected (EI), and exposed-to-removed (IR):
$$
\mathbf{N}_{g,t} = \left(N^{SE}_{g,t}, N^{EI}_{g,t}, N^{IR}_{g,t} \right)
$$
This counting process arises as the result of increments $\Delta \mathbf{N}_{g,t}$ added to an initial state $\mathbf{N}_{g,0}$:
$$
\begin{aligned}
...
In order to account for unobserved heterogeneity in susceptibility and exposure, we assume that elements of the vector $\mathbf{N}_{g,t}$ are distributed as beta-binomial random variables:
$$
\begin{aligned}
...
Then incident cases are denoted $\Delta N^{EI}_{g,t} = N^{EI}_{g,t} - N^{EI}_{g,t - 1}$ and $y_{g,t} \sim \text{Binomial}(\Delta N^{EI}_{g,t}, p)$.

In order to complete our model, we need to model changes between compartments, which we denote $\Delta N^{SE}_{g,t}$
