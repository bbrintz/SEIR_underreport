...
Crucially, $\pi^{SE}_{g,t},\pi^{EI}_{g,t},\pi^{IR}_{g,t}$ must be bounded in the intervals $[0,s_{g,t-1}], [0, e_{g,t-1}], [0, i_[g,t-1]]$ respectively, to faithfully approximate the counting process.
We enforce these bounds by creating latent variables $u^{SE}_{g,t},u^{EI}_{g,t},u^{IR}_{g,t}$ in $[0,1]$ and set $\pi^{SE}_{g,t} = u^{SE}_{g,t}s_{g,t}$,  $\pi^{EI}_{g,t} = u^{EI}_{g,t}e_{g,t}$, and $\pi^{IR}_{g,t} = u^{IR}_{g,t}i_{g,t}$.

The priors we use on the latent variables approximate the 
