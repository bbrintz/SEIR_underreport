...
transformed parameters {
  ...
  real<lower=0> obs_phi = 1 / square(obs_cv);
...
model {
  ...
  sig_beta ~ normal(0,.5); // normal(log(0.03), 0.5);
  log_obs_cv ~ normal(log(0.12), 0.2);

  i0_raw ~ normal(logit(3e-4), 1.0);
  gamma_raw ~ normal(logit(0.75), 0.5);
  p_raw ~ logistic(0,1);

  eta_raw ~ normal(logit(0.75), 0.5);
      ...
      if (i >= first[ct] && i <= last[ct]) {
        {
          ...
          real mu_ii = p * n_ii;
          ii[i,ct] ~ neg_binomial_2(mu_ii, obs_phi);
        }
      }
    }
  }
