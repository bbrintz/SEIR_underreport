...
tt <- cmdstan_model("SEIR_betabin_on_hier_ar1_beta_pbeta_zeros_v15_pois.stan")
...
out_dir <- "/private/tmp/seir_underreport_v15_pois_i0"
...
fit = tt$sample(data = dat, chains = 4,
                 ...
                 init = \() {list(u_t_logit_eta = matrix(rnorm(TT*N_C, 0,1), TT, N_C),
                                  ...
                                  log_i0_count = rnorm(N_C, log(20), 0.5),
...
fit$summary(c("eta_p_time", "average_p", "log_i0_count", "i0"))
...
mcmc_pairs(fit$draws(c("p", "sigma", "sig_beta[1]", "i0[1]", "i0[2]")),
           np = np_fit,
           pars = c(p_reference, "sigma", "sig_beta[1]", "i0[1]", "i0[2]"),
                  off_diag_args = list(size = 0.75))

#fit$output_files()=paste0(getwd(),"/",new_csv)

# To reload a previous v15 Poisson run, set new_csv to one of its output CSV files.
# new_csv <- paste0(out_dir, "/SEIR_betabin_on_hier_ar1_beta_pbeta_zeros_v15_pois-<timestamp>-1-<id>.csv")
