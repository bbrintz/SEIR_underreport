...
parameters {
  ...
  matrix[n_beta, N_C] raw_log_beta;

  real mu_log_beta;
  vector[n_glob_beta] z;
  real<lower=0> sigma; 
  vector<lower=0>[N_C] sig_beta;
//  real<lower=0> inv_obs_phi;
   
  matrix[TT, N_C] eta_p_t_x_c;  // unconstrained
  vector[n_p_pers] eta_p_time;  // unconstrained
  vector<lower=0>[N_C] sd_p_c;
  vector<lower=0, upper=1>[N_C] e0;  
...
transformed parameters {
  ...
  real phi = 2 * v - 1;
  matrix[TT, N_C] p;
  // real<lower=0> obs_phi = inv(inv_obs_phi);
  real rho_ei = inv_logit(rho_ei_raw);
  real rho_ir = inv_logit(rho_ir_raw);
  real obs_phi = inv(inv_obs_phi);
 ...
 //  i_t[n, ct] = i_t[n - 1, ct] + ei_t[n-1, ct] - ir_t[n-1, ct];
  ...
  for (ct in 1:N_C)
     p[first[ct]:last[ct],ct] = inv_logit(eta_p_time[p_idx[first[ct]:last[ct]]] +
                                 sd_p_c[ct] * eta_p_t_x_c[first[ct]:last[ct],ct]);
...
log_beta[glob_beta_start] = mu_log_beta/(1-phi)
  ...
  for (ct in 1:N_C) {
    log_beta_mat[first[ct]:last[ct], ct] = log_beta[first[ct]:last[ct]] 
                                           + sig_beta[ct] * raw_log_beta[idx_starts[ct]:idx_ends[ct],ct];
...
model {
  ...
  to_vector(raw_log_beta) ~ std_normal();

  for (ct in 1:N_C) {
    for (i in 1:(TT-1)) {
      if (i >= first[ct] && i <= last[ct]) 
        ii[i,ct] ~ neg_binomial(p[i,ct] * ei_t[i,ct] * pop_size[ct], obs_phi);
    }
  }
}
