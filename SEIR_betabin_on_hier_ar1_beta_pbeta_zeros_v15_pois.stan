...
transformed parameters {
  ...
  vector<lower=0, upper=1>[N_C] i0 = exp(log_i0_count) ./ pop_size;

  for (ct in 1:N_C) {
    // Historical i0 alignment: i0 is both the infectious state and the
    // incident E-to-I flow observed in the county's first modeled week.
  ...
  for (ct in 1:N_C) {
    for (n in (first[ct] + 1):TT) {
      ...
      if (t == first[ct]) {
        // The seeded first-week EI flow did not come out of modeled E.
...
model {
  ...
  // Independent lognormal priors on initial infectious counts:
  // median 20, with a broad 95% interval of approximately 1 to 380.
  log_i0_count ~ normal(log(20), 1.5);
