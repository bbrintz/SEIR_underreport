...
d1=read_csv("chlym.csv")

pop_size=d1$Population

d1=t(d1 %>% select(contains("X"),-X))
...
set.seed(123)
stan_output_dir <- file.path(tempdir(), "tst_folder_sig")
dir.create(stan_output_dir, recursive = TRUE, showWarnings = FALSE)
...
mcmc_pairs(fit$draws(c("p","phi","sigma","sig_beta","rho_ir","i0","mu_log_beta")), np = np_fit, pars = c("p","phi","rho_ir","mu_log_beta","i0[1]"),
...
plt_dat=obs %>% mutate(County=rep(unique(truth$County),each=12),date=date+1) %>%# filter(date<30) %>%
