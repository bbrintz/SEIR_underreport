...
d1[14,12] = 1
d1[11,6] = 1
tt <- cmdstan_model("SEIR_betabin_on_hier_ar1_beta_pbeta_zeros_v18_binom_approx.stan")
...
first_observed=ii[cbind(first,seq_len(N_C))]
...
out_dir <- "/private/tmp/seir_underreport_v18_binom_approx"
...
fit = tt$sample(data = dat, chains = 4,
                 ...
                 init = \() {list(u_t_logit_eta = matrix(rnorm(TT*N_C, 0,1), TT, N_C),
                                  ...
                                  eta_p_time = rnorm(ceiling((TT - 1) / dat$N_wks_per_period),
                                                     qlogis(0.2), 0.15),
                                  log_obs_cv = rnorm(1, log(0.10), 0.10),
                                  #kappa = runif(1, 0,1),
                                  ...
                                  # lambda0 = p * F0_count; initialize near the
                                  # first observed count only to aid adaptation.
                                  log_initial_reported_count =
                                    rnorm(N_C, log(pmax(first_observed, 1)), 0.1),
...
fit$summary(c("eta_p_time", "average_p", "obs_cv", "rho_ei", "rho_ir",
              "log_initial_reported_count", "initial_reported_count",
              "log_initial_ei_count", "initial_ei_count", "initial_ei",
              "initial_e", "initial_i"))
...
mcmc_pairs(fit$draws(c("p", "sigma", "sig_beta[1]", "rho_ei", "rho_ir",
                       "initial_reported_count[1]", "initial_ei_count[1]",
                       "initial_i[1]")),
           np = np_fit,
           pars = c(p_reference, "sigma", "sig_beta[1]", "rho_ei", "rho_ir",
                    "initial_reported_count[1]", "initial_ei_count[1]",
                    "initial_i[1]"),
...
# To reload a previous v18 binomial-approximation run, set new_csv to one of its output CSV files.
# new_csv <- paste0(out_dir, "/SEIR_betabin_on_hier_ar1_beta_pbeta_zeros_v18_binom_approx-<timestamp>-1-<id>.csv")
