...
tt <- cmdstan_model("SEIR_betabin_on_hier_ar1_beta_pbeta_zeros_v13_pois.stan")
...
# Initialize every county on the same date, four weeks before the earliest
# observed positive week. All county-weeks from this point onward—including
# leading and intervening zeros—contribute to the likelihood.
N_pre_detection_wks <- 4L
model_start <- max(2L, min(first) - N_pre_detection_wks)
...
dat$ii %>% as_tibble %>% gather(County,Cases) %>%
  mutate(date=rep(1:(TT-1),N_C),first=rep(first,each=TT-1)) %>% 
  ggplot(aes(x=date,y=Cases)) + geom_line() +
  geom_vline(aes(xintercept=first),color="red") +
  geom_vline(xintercept=model_start,linetype="dashed",color="blue") +
  facet_wrap(~County,scales="free_y") + theme_bw() +
...
# Initialize the non-centered hierarchy on the initial exposed count scale.

out_dir <- "/private/tmp/seir_underreport_v13_pois_global"
...
fit = tt$sample(data = dat, chains = 4,
                 ...
                 init = \() {list(u_t_logit_eta = matrix(rnorm(TT*N_C, 0,1), TT, N_C),
                                  ...
                                  w_t_logit_eta = matrix(rnorm(TT*N_C, 0,1), TT, N_C),
                                  #raw_log_beta_mat = matrix(runif(TT*N_C, -1, 1), TT, N_C),#matrix(rnorm(TT*N_C, 0, 1), TT, N_C),
                                  raw_log_beta = rnorm((TT - model_start) * N_C),
                                  eta_p_time = rlogis(ceiling((TT - 1) / dat$N_wks_per_period)),
                                  #kappa = runif(1, 0,1),
                                  v_raw=runif(1,1.5,2.5),
                                  z=rnorm(TT - model_start + 1,0,.25),
                                  mu_log_beta = rnorm(1, 0, .05),
                                  sigma = runif(1, .1, .25),
...
p_reference <- sprintf("p[%d,1]", model_start)
mcmc_pairs(fit$draws(c("p", "sigma", "sig_beta", "e0[1]", "e0[2]")), np = np_fit, pars = c(p_reference, "sigma", "sig_beta[1]", "e0[1]", "e0[2]"),
                  off_diag_args = list(size = 0.75))

#fit$output_files()=paste0(getwd(),"/",new_csv)

# To reload a previous v13 Poisson run, set new_csv to one of its output CSV files.
# new_csv <- paste0(out_dir, "/SEIR_betabin_on_hier_ar1_beta_pbeta_zeros_v13_pois-<timestamp>-1-<id>.csv")
...
dts=dat_final %>% filter(date<ymd("2020/8/19")) %>% 
ungroup() %>% rename(County="Admin2") %>% left_join(pop,by="County") %>% arrange(desc(Population_2020),date) %>%
dplyr::select(-Population_2020,-Latitude,-Longitude,-cases) %>% pivot_wider(names_from=County,values_from=new_cases)  %>% select(Date=date) %>% mutate(date=1:29)

plt_dat=obs %>% mutate(County=rep(unique(truth$County),each=29),date=date+1) %>% filter(date<30) %>%
left_join(truth,by=c("County","date")) %>% mutate(County=factor(County,levels=unique(truth$County))) #%>%



plt_dat %>% ggplot(aes(x=date,y=mean,ymin=lwr,ymax=upr)) + geom_point(color="blue") + 
geom_ribbon(alpha=.75,fill="lightblue") +
facet_wrap(~County,scales="free_y") + 
geom_point(data=dat$ii %>% as_tibble %>% gather(County,Cases) %>% mutate(date=rep(1:(TT-1),N_C),lwr=1,upr=1,County=factor(County,levels=unique(truth$County))) %>% filter(date<30),
aes(x=date,y=Cases),color="black") + geom_line() + theme_bw() + ylab("New Cases") + xlab("Date") + 
scale_x_continuous(breaks = seq(1, 29, by = 4), labels = substr(as.character(format(dts$Date, "%m/%d")),2,6)[seq(1, 29, by = 4)]) + 
theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8)) + 
theme(legend.position="none")

ggsave("utah_120525_bb.png",width=12,height=8)
